...
import static com.frontend.CreateStudent.getCount;
...
public class EditStudentInformation extends AppLayout {
    ...
    public EditStudentInformation() throws IOException {
        ...
        edit.addClickListener(event ->{
            ...
            StudentData student = selected.getStudentData();
            ...
            IntegerField communityServiceHours = new IntegerField("Community Service Hours");
            ...
            binder.forField(communityServiceHours)
                    .withValidator(new IntegerRangeValidator(
                            "Please Enter a Value Number of Hours", 0, null))
            ...
            save.addClickListener(e -> {
                ...
                if (binder.writeBeanIfValid(student)) {
                    try {
                        updateTracker(selected, student);
                    } catch (Exception ex) {
                        Notification.show("Error Ocurred, Please Try Again \n" + ex.getMessage());
                        ex.printStackTrace();
                    }
                    try {
                        FileMethods.removeStudent(selected);
